...
macro_rules! define_lending_errors {
    (
        ...
        impl LendingError {
            /// Every variant, in declaration order. Generated by the macro from the
            /// same list that defines the enum, so it can never drift from it —
            /// adding a variant extends this automatically.
            pub const ALL: &[LendingError] = &[$(Self::$variant),*];
        ...
        // Empty variant list would make the roundtrip test vacuous.
        const _: () = assert!(!LendingError::ALL.is_empty());
    };
}
...
define_lending_errors! {
    ...
    OrderIndexOutOfBounds = 123 => "Order of the given index cannot exist",
...
mod tests {
    ...
    /// Every variant round-trips, the codes form one contiguous block starting at
    /// `ANCHOR_ERROR_BASE`, and nothing just outside the block decodes. All bounds
    /// derive from `LendingError::ALL`, so adding a variant extends coverage with no
    /// magic numbers to keep in sync.
    #[test]
    fn test_all_codes_roundtrip() {
        let mut checked = 0usize;
        for (i, &err) in LendingError::ALL.iter().enumerate() {
            let code = ANCHOR_ERROR_BASE + i as u32;
            // Offset `i` must equal `code - ANCHOR_ERROR_BASE`: catches a skipped or
            // duplicated offset literal in the macro list.
            assert_eq!(
                err.error_code(),
                code,
                "variant #{i} ({err:?}) is not contiguous"
            );
            assert_eq!(
                LendingError::from_error_code(code),
                Some(err),
        ...
        // Boundaries move with `ALL.len()`: nothing valid just below or above the block.
        assert_eq!(LendingError::from_error_code(ANCHOR_ERROR_BASE - 1), None);
        let just_past = ANCHOR_ERROR_BASE + LendingError::ALL.len() as u32;
        assert_eq!(LendingError::from_error_code(just_past), None);
    }
}
